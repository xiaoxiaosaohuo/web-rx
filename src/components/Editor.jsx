      ...
      {files.map((f, index) => (
        ...
        >
          ...
          <button
            ...
            onClick={(e) => {
              if (
                !f.ref.editorState.readOnly &&
                !confirm('Close ' + f.name + '?')
              ) {
                e.stopPropagation();
                return;
              }
...
export function Editor({ webR, terminalInterface, filesInterface }) {
  ...
  React.useEffect(() => {
    ...
    webR.init().then(async () => {
      ...
      completionMethods.current = {
        ...
        retrieveCompletions: await shelter.evalR(
  ...
  const completion = React.useCallback(async (context) => {
    ...
    const { from, to, text } = context.matchBefore(/[a-zA-Z0-9_.:]*/) ?? {
      from: 0,
      to: 0,
      text: '',
    };
  ...
  React.useEffect(() => {
    filesInterface.openFileInEditor = (name, path, readOnly) => {
      ...
      return webR.FS.readFile(path).then((data) => {
        syncActiveFileState();
        const updatedFiles = [...files];
        const extensions = name.toLowerCase().endsWith('.r')
          ? editorExtensions
          : [];
  ...
  return (
    ...
    >
      ...
      <p style={{ display: 'none' }} id="editor-desc">
        ...
        editor. The editor has been configured so that the Tab key controls the
        indentation of code. To move focus away from the editor, press the
        Escape key, and then press the Tab key directly after it. Escape and
        then Shift-Tab can also be used to move focus backwards.
      </p>
      <div
        role="toolbar"
